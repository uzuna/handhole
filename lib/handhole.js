...
// addi tional Module
const Hopper = require('./modules/hopper')
const Valve = require('./modules/valve')
const FlowMater = require('./modules/flowMater')
...
HandHole.prototype.pipe = function(target, nextpipe){
	...
	if(arguments.length === 1){
		...
		if(list.length > 1){
			...
			list = this.term().end.filter(function (d) {
				return isstream.isReadable(d.obj);
			});
			list = list.concat(this.term().alone.filter(function (d) {
				return isstream.isReadable(d.obj);
			}));
		}
