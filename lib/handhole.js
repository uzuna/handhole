...
HandHole.prototype.flowMater = function(target, option){
	if(arguments.length < 1) return genFlowMater();
	var fm = genFlowMater(option);
	this.insert(target, fm);
	return fm;
}

module.exports.flowMater = genFlowMater;
...
HandHole.prototype.valve = function(target, option){
	if(arguments.length < 1) return genValve();
	var vl = genValve(option)
	if(target === null) return vl;
	this.insert(target, vl);
	return vl;
}

module.exports.valve = genValve;
...
HandHole.prototype.insert = function(target, stream){
	...
	// Auto head Insert
	if(isstream.isReadable(target)){
		stream = target;
		var start = this.term().start;
		start.filter(function (d){
	...
	if(!isstream.isWritable(dest.obj)) throw new Error("Cannot pipe to Readable pipe")
...
HandHole.prototype.pipe = function(target, nextpipe){
	var self = this;
	if(arguments.length === 1 && isstream.isWritable(target)){
		// 最後尾に追加する
		nextpipe = target
		var list = this.term().end.filter(function (d) {
			return isstream.isReadable(d.obj);
		});
		// if(list.length > 1) throw new Error("multi end termination")
		if(list.length < 1) return 0;
		list.forEach(function (d) {
			if(d.name === "garbage"){
				self.insert(d.id, nextpipe);
			}else{
				self.pipe(d.id, nextpipe);
			}
		})
		return this._update(nextpipe);
	}
